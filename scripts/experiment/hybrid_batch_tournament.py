...
class HybridBatchTournament:
    ...
    def _build_batch_request(
        ...
        if config.provider == "anthropic":
            ...
            # build_google_request returns (key, request_dict), unpack to get just the dict
            _, request = BatchRequestBuilder.build_google_request(
                key=custom_id,
                system_prompt=system_prompt,
                user_prompt=user_prompt,
                model=config.model,
        ...
        # All providers now return (provider_name, request_dict) consistently
    ...
    def run_round(self, round_num: int, verbose: bool = True) -> int:
        ...
        # Initialize results containers BEFORE they're used
        all_results = {}
        gemini_results = {}  # Must exist even if Google block is skipped
